...
public class DirectoryDAOImpl extends MySQL implements DirectoryDAO {
    ...
    public DirectoryClass getDirectoryById(int dirId) {
        Connect();
        DirectoryClass directory = null; //if no records returned we dont want to use mem
        try {
    ...
    public List<DirectoryClass> getDirectoryList() {
        Connect();
        List<DirectoryClass> directoryList = new ArrayList<DirectoryClass>(); //if no records returned we dont want to use mem
        try {
            String sp = "{call sp_GetDirectory(?,?)}";
            CallableStatement cStmt = connection.prepareCall(sp);
    ...
    private static DirectoryClass hydrateObject(ResultSet rs) throws SQLException{
        /*
        dirId - int 1
